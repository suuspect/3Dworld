...
function skybox(){
  ...
  let materialArray = [];
  let texture_ft = new THREE.TextureLoader().load('skybox/blizzard_ft.jpg');
  let texture_bk = new THREE.TextureLoader().load('skybox/blizzard_bk.jpg');
  let texture_up = new THREE.TextureLoader().load('skybox/blizzard_up.jpg');
  let texture_dn = new THREE.TextureLoader().load('skybox/blizzard_dn.jpg');
  let texture_rt = new THREE.TextureLoader().load('skybox/blizzard_rt.jpg');
  let texture_lf = new THREE.TextureLoader().load('skybox/blizzard_lf.jpg');
...
function objectLoader(){
  ...
  scene.add( mesh );
   ...
   //img.onload = function () {
       ...
       let text1 = new THREE.TextureLoader().load('textures/Seoul.jpg');
...
function modelLoader(){
  ...
  let p1 = loadModel('models/Godzilla.glb').then(result => {  model1 = result.scene.children[0]; });
  let p2 = loadModel('models/Chicken.glb').then(result => {  model2 = result.scene.children[1]; });
  let p3 = loadModel('models/Bird.glb').then(result => {  model3 = result.scene.children[0]; });
  ...
  Promise.all([p1,p2,p3,p4,p5]).then(() => {
     ...
     //do something to the model2
     model2.position.set(85,23.5,5);
     model2.rotation.set(0.05*Math.PI/2,0.05*Math.PI/2,0);
     //model2.castShadow = true;
     var scaleSizeModel2 = 0.5;
     model2.scale.set(scaleSizeModel2,scaleSizeModel2,scaleSizeModel2);
         var texture2 = textureLoader.load('textures/chicken.png');
         var material2 = new THREE.MeshBasicMaterial();
         //bot = gltf.scene.children[0];
         //material2.metalness = 0;
         material2.map = texture2;
         model2.traverse( function ( model2 ) {
           if ( model2 instanceof THREE.Mesh ) {
                model2.material = material2;
           }
         } );

    model3.position.set(20,0.1,-40);
    var scaleSizeModel3 = 0.09;
    model3.scale.set(scaleSizeModel3,scaleSizeModel3,scaleSizeModel3);
        var texture3 = textureLoader.load('textures/bird.png');
        var material3 = new THREE.MeshBasicMaterial();
        //bot = gltf.scene.children[0];
        //material2.metalness = 0;
        material3.map = texture3;
        model3.traverse( function ( model3 ) {
          if ( model3 instanceof THREE.Mesh ) {
               model3.material = material3;
          }
        } );


    var scaleSizeModel4 = 200;
    model4.scale.set(scaleSizeModel4,scaleSizeModel4,scaleSizeModel4);
    model4.position.set(100,112,0);
